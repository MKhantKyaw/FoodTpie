...
@section('content')
    <?php
      ...
      foreach ($orders as $order) {
        ...
        $time = explode(':', $orderTime);
        $hour = $time[0];
        $dateComp = explode('.', $orderDate);
        $orderObj = (object) array('hour' => $hour , 'day' => $dateComp[0] , 'month' => $dateComp[1] , 'year' => $dateComp[2] , 'earning' => $order->total_price);
      ...
      foreach ($orderObjArr as $orderObj) {
        if($orderObj->year == $year){
          $annualEarning+= $orderObj->earning;
          if($orderObj->month == $month){
            $monthlyEarning+= $orderObj->earning;
          }
        }
        if($orderObj->year > $year){
          $pendingOrders++;
        }else if($orderObj->year == $year){
          if($orderObj->month > $month){
            $pendingOrders++;
          }else if($orderObj->month == $month){
            if($orderObj->day > $day){
              $pendingOrders++;
            }else if($orderObj->day == $day){
              if($orderObj->hour > $hour){
                $pendingOrders++;
              }
            }
          }
                ...
                <div class="card-body">
                  <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                      ...
                      <div class="h5 mb-0 font-weight-bold text-gray-800">${{ $annualEarning }}</div>
                    </div>
                    <div class="col-auto">
                ...
                <div class="card-body">
                  <div class="row no-gutters align-items-center">
                    <div class="col mr-2">
                      ...
                      <div class="row no-gutters align-items-center">
                        <div class="col-auto">
                          <div class="h5 mb-0 mr-3 font-weight-bold text-gray-800">{{ $productCnt }}</div>
                        </div>
                      </div>
                    </div>
                    <div class="col-auto">
